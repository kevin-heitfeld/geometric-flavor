...
\subsection{Explicit Calculation for $\mathbb{P}_{11226}[12]$}

For our specific Calabi--Yau threefold $X = \mathbb{P}_{11226}[12]$, the relevant topological data are:
\begin{itemize}
    \item Hodge numbers: $(h^{1,1}, h^{2,1}) = (1, 272)$.
    \item Second Chern class: $c_2(TX) = 66 H^2$ where $H$ is the hyperplane divisor.
    \item Fourth Chern class (squared): $c_2^2 = 4356 H^4$.
    \item Euler characteristic: $\chi(X) = -542$.
\end{itemize}
...
For our baseline moduli values $\tau = 1.2 + 0.8i$, $\rho = 1.0 + 0.5i$, the numerical computation yields:
\begin{align}
