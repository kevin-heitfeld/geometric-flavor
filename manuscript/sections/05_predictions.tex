...
This value arises from the complex phases in the seesaw mechanism, which in turn originate from the modular form $\eta(\tau)$ at $\tau = 0.5 + 1.6i$:
\begin{equation}
    \arg[\eta(\tau)] = \arctan\left(\frac{\text{Im}[\eta(\tau)]}{\text{Re}[\eta(\tau)]}\right) \approx 10.1^\circ.
\end{equation}
After propagating through the Majorana mass matrix and seesaw (Eq.~\ref{eq:seesaw}), this phase gets enhanced by factors related to the mass hierarchy, yielding the final $\delta_{CP}^\nu \approx 206^\circ$.
