...
Our results are derived for a specific Calabi--Yau threefold: the toroidal orbifold $T^6/(\ZZ_3 \times \ZZ_4)$ with Hodge numbers $(h^{1,1}, h^{2,1}) = (3, 75)$ after blow-up resolution. How generic are our conclusions?
...
\begin{itemize}
    ...
    \item \textbf{Discrete symmetries}: Our toroidal orbifold $T^6/(\ZZ_3 \times \ZZ_4)$ possesses $\ZZ_3 \times \ZZ_4$ discrete isometries that naturally give rise to modular flavor symmetries ($\Gamma_0(3)$ and $\Gamma_0(4)$), which determine the hierarchical structure of Yukawa matrices~\cite{Ibanez:2012zz}.
    \item \textbf{Complex structure moduli space}: The dimension $h^{2,1} = 75$ provides sufficient freedom to tune moduli for optimal flavor agreement. Toroidal orbifolds generally have intermediate $h^{2,1}$ values between simple tori and complex hypersurfaces.
\end{itemize}

\paragraph{Landscape Perspective.}
String theory predicts $\sim 10^{500}$ flux vacua on different CY geometries~\cite{Ashok:2003gk}. Our choice of $T^6/(\ZZ_3 \times \ZZ_4)$ is not unique---many other toroidal orbifolds or CY manifolds could produce similar flavor structure. However, the requirement of Standard Model chirality (three generations, correct quantum numbers) and realistic moduli stabilization dramatically reduces the viable subset~\cite{Taylor:2015xtz}. Our orbifold satisfies:
\begin{equation}
\chi(T^6/(\ZZ_3 \times \ZZ_4)) = -144,
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Loop Corrections}: We work at tree level in string perturbation theory. Rigorous calculations of one-loop $\alpha'$ corrections (gauge thresholds and moduli exchange) and worldsheet instantons show $\sim 2\%$ shifts to Yukawa couplings, with $\chi^2/\text{dof}$ increasing from $1.18 \to 1.52$ (well within acceptable range $< 2.0$). The framework is robust against quantum corrections. See Appendix~\ref{app:loop_corrections} for details.
...
\begin{itemize}
    ...
    \item \textbf{Calabi--Yau choice} is not unique; many toroidal orbifolds or CY manifolds could work, but $T^6/(\ZZ_3 \times \ZZ_4)$ is explicit and consistent.
