...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{figures/modular_inflation_analysis.png}
\caption{\textbf{$\alpha$-attractor inflation from the blow-up modulus $\sigmamod$.} Top panel shows the K\"ahler potential $K = -3\log(\sigmamod + \bar{\sigmamod})$ driving slow-roll inflation. Middle panel displays the scalar potential in canonical field space $\phi = (\sqrt{3}/2)\log\sigmamod$, with slow-roll parameters $\varepsilon$ and $\eta$ shown in the bottom panel. The predicted spectral index $n_s = 0.9667$ and tensor-to-scalar ratio $r = 0.0033$ (for $N=60$ e-folds) match Planck 2018 observations at the $1\sigma$ level.}
\label{fig:inflation}
\end{figure}
