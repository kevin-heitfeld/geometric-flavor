...
\begin{equation}
\boxed{\Omega_{\text{PNGB}} \approx 0.726, \quad w_0 \approx -0.98, \quad w_a = 0}
\end{equation}

This is a \textit{structural feature} of PNGB quintessence with $f \sim \MPlank$, not a tunable parameter. Rather than forcing this to match the observed $\OmegaDE = 0.685$ (which would require suppression mechanisms and parameter scanning), we interpret this as the natural contribution from the modular sector:
\begin{equation}
\Omegazeta \approx 0.068 \quad \text{(subdominant dynamical component)}
\end{equation}

with the remaining $\sim 90\%$ provided by vacuum energy. This reframing is developed in Section~\ref{sec:two_component}.
