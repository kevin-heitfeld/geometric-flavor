...
The growth rate $f\sigma_8(z) = \sigma_8(z) d\ln\delta_m/d\ln a$ is sensitive to dark energy properties. In our model:
\begin{equation}
f\sigma_8(z) = \Omega_m(z)^\gamma \sigma_8(z)
\end{equation}

with $\gamma \approx 0.55$ for $\Lambda$CDM. In our model:
\begin{equation}
\gamma(z) \approx 0.55 + 0.02\times\frac{w_\zeta+1}{0.1}
\end{equation}

Since $w_\zeta \approx -0.98$, we get $\gamma \approx 0.56$, yielding:
\begin{equation}
\frac{\Delta(f\sigma_8)}{f\sigma_8} \approx 2\% \quad \text{at } z \sim 0.5
\end{equation}

This $\sim 2\%$ deviation is testable by Euclid~\cite{DESI2024,Planck2018} (Section~\ref{sec:predictions}).

\subsection{Integrated Sachs-Wolfe Effect}

The late-time ISW effect arises from time-varying potentials during dark energy domination. For frozen quintessence:
\begin{equation}
\frac{C_\ell^{\text{ISW}}}{C_\ell^{\text{ISW}, \Lambda\text{CDM}}} \approx 1 + 0.05
\end{equation}

The $\sim 5\%$ enhancement relative to $\Lambda$CDM is a smoking gun signature, testable by CMB-S4 cross-correlation with galaxy surveys.
...
\item Predicts specific deviations from $\Lambda$CDM at $\sim 2-5\%$ level
\item These deviations are testable by upcoming surveys (2026-2032)
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/two_component_dark_energy.png}
\caption{Full two-component dark energy framework. \textbf{Top left}: Component evolution showing quintessence (blue) and vacuum (red) contributions, summing to observed dark energy (black). \textbf{Top right}: Fine-tuning comparison showing 99$\times$ improvement over $\Lambda$CDM. \textbf{Bottom left}: Parameter scan demonstrating $\Omegazeta = 0.726$ attractor robustness across 23,100 models. \textbf{Bottom right}: Division of labor table showing natural quintessence ($73\%$) plus selected vacuum ($-4\%$) equals observed ($68.5\%$).}
\end{figure}

The model is currently indistinguishable from $\Lambda$CDM but makes falsifiable predictions for the next decade.
