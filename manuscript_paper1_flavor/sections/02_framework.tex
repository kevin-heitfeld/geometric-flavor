...
\begin{itemize}
    \item Three complex structure moduli $U^i$ and one dilaton $\tau = C_0 + i/g_s$\footnote{\textbf{Notation}: Throughout this paper, $g_s$ denotes the \emph{dilaton coupling} $g_s = e^\phi$ controlling string loop expansion. The 4D effective gauge couplings include additional threshold corrections and are discussed in the companion paper on string theory origin~\cite{Heitfeld:2025origin}.}
