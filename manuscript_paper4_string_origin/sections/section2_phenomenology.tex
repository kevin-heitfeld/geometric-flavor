...
\end{equation}
where $M_D$ (Dirac) has democratic structure from modular symmetry, and $M_R$ (Majorana) has hierarchical structure with modular weights $k = (5, 3, 1)$ following the universal $\Delta k = 2$ pattern. Complex CP phases in $M_D$ generate neutrino CP violation.

With $\tau = 2.69i$ and $\sim$12 real parameters, we fit:
\begin{itemize}
\item Charged lepton masses: $m_e/m_\mu/m_\tau \approx 1/200/3477$ $\checkmark$
\item Neutrino mass differences: $\Delta m_{21}^2 = 7.5 \times 10^{-5}$ eV$^2$, $\Delta m_{31}^2 = 2.5 \times 10^{-3}$ eV$^2$ $\checkmark$
\item Absolute neutrino masses: $m_1 = 1.2$ meV, $m_2 = 8.7$ meV, $m_3 = 50$ meV (testable)
\item PMNS mixing angles: $\theta_{12} \approx 34^\circ$, $\theta_{23} \approx 49^\circ$, $\theta_{13} \approx 8.6^\circ$ $\checkmark$
\item CP phase: $\delta_{\text{CP}} \approx 206^\circ$ (large CP violation) $\checkmark$
\item Neutrinoless double-beta decay: $\langle m_{\beta\beta} \rangle = 10.5 \pm 1.5$ meV (testable by LEGEND-1000)
\end{itemize}

\textbf{Key point}: A single modular parameter $\tau$ describes 13 lepton observables (3 charged lepton masses + 2 neutrino splittings + 3 PMNS angles + 1 CP phase + 3 absolute neutrino masses + 1 $\langle m_{\beta\beta} \rangle$). Traditional models need $\sim$15-20 free parameters.

\textbf{Universal $\Delta k = 2$ pattern}: All fermion sectors follow the same modular weight hierarchy:
\begin{itemize}
\item Charged leptons: $k = (8, 6, 4)$ with $\Delta k = 2$
\item Right-handed neutrinos: $k = (5, 3, 1)$ with $\Delta k = 2$
\item Up quarks: $k = (6, 4, 2)$ with $\Delta k = 2$
\item Down quarks: $k = (4, 2, 0)$ with $\Delta k = 2$
\end{itemize}
This universality is a geometric prediction from D7-brane wrapping numbers on the $T^6/(\mathbb{Z}_3 \times \mathbb{Z}_4)$ orbifold.
...
\begin{itemize}
\item Quark masses: $m_u/m_c/m_t \approx 1/600/85000$ and $m_d/m_s/m_b \approx 1/20/900$ $\checkmark$
\item CKM mixing angles: $\theta_{12}^{\text{CKM}} \approx 13^\circ$, $\theta_{23}^{\text{CKM}} \approx 2.4^\circ$, $\theta_{13}^{\text{CKM}} \approx 0.2^\circ$ $\checkmark$
\item CP phase: $\delta_{\text{CP}}^{\text{CKM}} \approx 70^\circ$ $\checkmark$
\end{itemize}
