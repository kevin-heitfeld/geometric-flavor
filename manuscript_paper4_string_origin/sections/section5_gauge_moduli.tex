...
\subsubsection{Revised dilaton determination}
\label{sec:dilaton_determination}

The range $g_s \sim 0.5\text{--}1.0$ from gauge unification represents a \textit{first estimate} that must be refined by considering the proper gauge kinetic function structure and dilaton stabilization.

\textbf{Proper gauge kinetic function}:  
Recall from Eq.~\eqref{eq:gauge_kinetic_structure} that $f_a = n_a T + \kappa_a S$, not the simplified $f = T/g_s$. The physical gauge coupling is:
\begin{equation}
\frac{1}{g_a^2(M_s)} = n_a \,\text{Re}(T) + \kappa_a \,\text{Re}(S)
\end{equation}

With observed $\alpha_{\text{GUT}}^{-1} \sim 25$ after threshold corrections, $\text{Re}(T) \sim 0.8$, $n_a \sim 1$, and $\kappa_a \sim 1$:
\begin{equation}
25 \sim 0.8 + \text{Re}(S) \implies \text{Re}(S) \sim 24
\end{equation}

If the dilaton modulus has comparable real and imaginary parts (typical for stabilized moduli), this gives:
\begin{equation}
\text{Im}(S) \sim 20\text{--}30 \implies g_s = \frac{1}{\text{Im}(S)} \sim 0.03\text{--}0.05
\end{equation}

**KKLT dilaton stabilization**:  
In the KKLT framework, the dilaton is stabilized by non-perturbative effects (gaugino condensation on hidden sector branes):
\begin{equation}
W_{\text{np}} = B \,e^{-S/N}
\end{equation}
where $N$ is the rank of the hidden gauge group (typically $N = 5\text{--}10$).

Minimizing the full scalar potential including loop corrections gives a dilaton VEV:
\begin{equation}
\langle S \rangle \sim N \times \ln\left(\frac{M_P}{m_{3/2}}\right) \sim 5\text{--}15 \quad \text{(for $N=5\text{--}10$, $m_{3/2} \sim 10^2$ TeV)}
\end{equation}

This yields:
\begin{equation}
g_s \sim 0.07\text{--}0.2
\end{equation}

**Convergence of three methods**:  
Three independent approaches point to a consistent value:
...
\hline
Gauge coupling (corrected DBI formula) & $0.03\text{--}0.05$ \\
KKLT stabilization (estimated) & $0.07\text{--}0.2$ \\
Dark energy SUGRA corrections (Paper 3) & $0.10\text{--}0.14$ \\
\hline
\end{tabular}
\end{center}

Taking the geometric mean and accounting for uncertainties:
\begin{equation}
\boxed{g_s = 0.10 \pm 0.05}
\label{eq:gs_final}
\end{equation}

This places the framework in the \textbf{weak coupling regime}, where perturbative string theory is manifestly valid. The ±50\% uncertainty reflects the schematic nature of the KKLT calculation and will be reduced through explicit flux choices and geometric details in future work (see Appendix~\ref{app:dilaton_stabilization}).

\textbf{Clarification on moduli}:  
It is crucial to distinguish the phenomenological complex structure modulus $U_{\text{eff}} = 2.69i$ (from flavor physics) from the axio-dilaton $S = C_0 + i/g_s$ (controlling string coupling). These are \textit{independent} moduli in the K\"ahler potential:
\begin{equation}
K = K_{\text{CS}}(U) + K_{\text{K}}(T) + K_{\text{dil}}(S)
\end{equation}
The value $g_s \approx 0.10$ comes from dilaton stabilization and gauge coupling consistency, \textit{not} from the relation $g_s = 1/\text{Im}(U)$ which applies only in specific F-theory limits.

\subsection{K\"ahler Modulus from Threshold Corrections}
...
$g_s = 1/\text{Im}(S)$ & String coupling & $0.10 \pm 0.05$ & Dilaton stabilization + gauge \\
