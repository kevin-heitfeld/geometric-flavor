...
By incorporating right-handed neutrinos as open-string modes on bulk D7-branes with Majorana masses $M_N \sim 10^{10}$--$10^{11}~\text{GeV}$ from modular forms with weights $k_\nu = (5, 3, 1)$ (following the universal $\Delta k = 2$ pattern), we naturally obtain small neutrino masses $m_\nu \sim \mathcal{O}(0.01~\text{eV})$ via the Type I seesaw. The democratic Dirac mass matrix (predicted by modular symmetry) produces large PMNS mixing angles, while the hierarchical Majorana masses generate the observed mass splittings. The framework correctly predicts the normal mass ordering, all three mixing angles within $1\sigma$ ($\theta_{12}^\nu \approx 34^\circ$, $\theta_{23}^\nu \approx 49^\circ$, $\theta_{13}^\nu \approx 8.6^\circ$), and provides testable predictions for absolute neutrino masses.
