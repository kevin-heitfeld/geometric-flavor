...
$w_0$ & $-1.03 \pm 0.03$ & $-1$ (exact) & $-0.994$ \\
...
The growth rate $f\sigma_8(z) = \sigma_8(z) d\ln\delta_m/d\ln a$ is sensitive to dark energy properties. With SUGRA-corrected quintessence ($\Omega_\zeta^{(\text{SUGRA})} \approx 0.690$), the modified expansion history affects structure growth:
\begin{equation}
...
The late-time ISW effect arises from time-varying potentials during dark energy domination. For frozen quintessence with $\Omega_\zeta^{(\text{SUGRA})} = 0.690$ and $w \approx -0.985$:
\begin{equation}
...
\textbf{Planck 2018}:
\begin{itemize}
\item $w_0 = -1.03 \pm 0.03$ (consistent with our $-0.994$)
...
Our model with $w_0 = -0.994$, $w_a = 0$ lies well within current uncertainties. The DESI hint of $w_a < 0$ is not statistically significant and could be systematic.
