...
\begin{itemize}
\item Closed string sector: No chiral matter $\times$
\item Open string sector: Must provide all SM fermions $\checkmark$
\end{itemize}
...
\item \textbf{D7-branes}: Wrap 4-cycles in the Calabi-Yau
   ...
   \item \textbf{This is what we use} $\checkmark$
   \end{itemize}
\end{enumerate}
...
\begin{itemize}
\item Three chiral generations (matches experiment) $\checkmark$
\item Correct gauge quantum numbers for quarks and leptons $\checkmark$
\item Framework for modular flavor symmetry (next section) $\checkmark$
\item Order-of-magnitude consistency with phenomenology $\checkmark$
\end{itemize}
